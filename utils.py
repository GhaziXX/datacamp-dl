...
import json
import re
from resources import download_course_data
from classes import Template


def download_track(track, folder, videos_download, data_download, slides_download):
    ...
    for i, link in enumerate(all_links):
        download_course(link, folder, videos_download, data_download, slides_download, i + 1)


def download_track_by_link(link, folder, videos_download, data_download, slides_download):
...
def download_course(url, folder, videos_download, data_download, slides_download, number=None):
    ...
    if videos_download:
        download_videos(course_id, os.path.join(
            folder, title))
    if data_download:
        download_course_data(con, url, os.path.join(
            folder, title))
